...
#define FONT_SANS "FreeSans.ttf"
#define FONT_MONO "FreeMonoBold.ttf"
...
void drawTextColor(SDL_Surface* screen,
              ...
              uint32_t fc,
              uint32_t bc,
              char* font_file /* [ FONT_SANS | FONT_MONO ] */
    )
{
		...
		double value)
{
	...
	for ( i = 0; i <= (max-min)/major_tick; i++) {
		...
		if (!(i == 0 && theta0 == 0)) {
			...
			drawTextColor(screen,buf,text_size,valx3,valy3,GREEN,BLACK,FONT_SANS);
		}
	}
	...
	// units
	drawTextColor(screen,units,text_size,xpos,ypos+r/3,GREEN,BLACK,FONT_SANS);
	

	// digital
	sprintf(buf,"%1.2f",value);
	drawTextColor(screen,buf,text_size,xpos,ypos+r/5,GREEN,BLACK,FONT_MONO);

}



void drawLCD(SDL_Surface* screen, 
    int center_x, int center_y,
    char* buf
    )
{
#define LCD_X_CHARS 20
#define LCD_Y_CHARS 4
#define LCD_X_PIX 5
#define LCD_Y_PIX 7
#define scale 4
  int charwidth = LCD_X_PIX*scale;
  int charheight = LCD_Y_PIX*scale;
  int width = charwidth*(LCD_X_CHARS);
  int height = charheight*(LCD_Y_CHARS);
  ...
  char charbuf[2];
  charbuf[1] = 0;
  boxColor(screen, left-charwidth, top-charwidth, right+charwidth, bottom+charwidth, GREY);
  boxColor(screen, left-charwidth/2, top-charwidth/2, right+charwidth/2, bottom+charwidth/2, DARK_GREEN);
  for (int y=0; y<LCD_Y_CHARS; y++) {
    for (int x=0; x<LCD_X_CHARS; x++) {
      charbuf[0] = buf[x+y*LCD_X_CHARS];
      charbuf[1] = 0;
      drawTextColor(screen,charbuf,
          charheight*0.9,left+(x+0.5)*charwidth,top+(y+0.5)*charheight,
          GREEN,DARK_GREEN,FONT_MONO);
    }
  }
}
